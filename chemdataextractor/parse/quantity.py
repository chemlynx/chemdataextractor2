...
magnitudes_dict: MagnitudeDict = {
    ...
    R("M(ega)?", group=0): 6.0,
    R("G(iga)?", group=0): 9.0,
    R("T(era)?", group=0): 12.0,
    R("m(illi)?", group=0): -3.0,
    R("µ|μ|(micro)|(mu)|\u03bc", group=0): -6.0,
    R("n(ano)?", group=0): -9.0,
    R("p(ico)?", group=0): -12.0,
}
...
def value_element(units=None, activate_to_range=False):
    ...
    fraction = R(r"^(([\+\-–−]?\d+/\d+)|(\<nUm\>))$") | (
        R(r"^(([\+\-–−]?\d+)|(\<nUm\>))$") + R(r"^/$") + R(r"^((\d+)|(\<nUm\>))$")
    ).add_action(merge)
    number = spaced_power_number | fraction | pure_number
    joined_range = R(r"^[\+\-–−~∼˜]?\d+(([\.・,\d])+)?[\-–−~∼˜]\d+(([\.・,\d])+)?$")(
        "raw_value"
    ).add_action(merge)
    ...
    value_single = (Optional(R(r"^[~∼˜\<\>≥]$")) + Optional(R(r"^[\-–−]$")) + number)(
        "raw_value"
    ).add_action(merge)
    value = (
...
def _clean_value_string(string):
    ...
    if len(split_by_comma) != 1:
        if len(split_by_comma) == 2:
            if "." not in split_by_comma[1] and len(split_by_comma[1]) != 3:
...
def _extract_brackets_error(string):
    ...
    if len(split_by_brackets) == 4:
        ...
        return error_unadjusted * 10.0**magnitude
    else:
...
def _split(string):
    ...
    for element in split_by_bracket:
        if element is not None:
            ...
            # Also merge numbers
            if (
                re.match(_fraction_or_decimal_pattern, element)
                or (
                    final_list != []
                    and re.match(_division_pattern, final_list[-1])
                    and not re.match(_end_bracket_pattern, element)
                    and not re.match(_open_bracket_pattern, final_list[-1])
                )
...
def _remove_subranges(ranges):
    ...
    new_ranges = [
        original_range
        for index, original_range in enumerate(ranges)
        if index not in should_remove_indices
    ]
    return new_ranges
    ...
    while i in range(len(powers)):
        if powers[i][1] == "(":
            ...
            current_powers.append(
                (powers[i][0], powers[i][1], powers[i][2] * base_power, powers[i][3])
            )
            i += 1
...
def _find_units(powers_cleaned, dimensions, strict):
    ...
    if strict:
        if (
            ...
            and end_unit.dimensions == dimensions
            and len(unassociated_elements) == 0
        ):
            return end_unit
        else:
            if end_unit is None:
            ...
            raise TypeError(
                "Parsed with Dimensions "
                + str(end_unit.dimensions)
                + ", expected"
                + str(dimensions)
            )
    else:
