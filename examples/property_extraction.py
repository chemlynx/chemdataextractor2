...
from chemdataextractor.model.model import (
    Compound, MeltingPoint, BoilingPoint, GlassTransition, 
    Solubility, Density, IrSpectrum, NmrSpectrum
)
...
def melting_point_extraction():
    ...
    for i, mp in enumerate(melting_points, 1):
        ...
        if mp.value:
            ...
            print(f"     Parsed value: Could not parse")
    ...
    for compound in compounds:
        if compound.names:
            ...
            if hasattr(compound, 'melting_point') and compound.melting_point:
                ...
                print(f"     No associated melting point found")
...
def range_and_uncertainty_handling():
    ...
    for i, mp in enumerate(melting_points, 1):
        ...
        if '±' in raw_text or '+-' in raw_text:
            print(f"     Type: Explicit uncertainty")
        elif '-' in mp.raw_value and not mp.raw_value.startswith('-'):
            print(f"     Type: Range measurement")  
        elif '~' in raw_text or 'approximately' in raw_text:
            print(f"     Type: Approximate value")
        elif '>' in raw_text or '<' in raw_text:
            print(f"     Type: Inequality")
        else:
            print(f"     Type: Point measurement")
...
def solubility_and_qualitative_properties():
    ...
    solubility_records = [r for r in records if r.__class__.__name__ == 'Solubility' or 
                         hasattr(r, 'solubility') or 'solub' in str(r).lower()]
    ...
    solubility_terms = [
        ...
        'dissolves', 'sparingly', 'freely', 'slightly',
        'highly soluble', 'very soluble', 'completely soluble'
    ]
    ...
    qualitative_patterns = [
        'slightly soluble', 'highly soluble', 'very soluble',
        'sparingly soluble', 'freely soluble', 'miscible', 'immiscible'
    ]
...
def property_validation_and_quality():
    ...
    compounds_with_mp = len([c for c in compounds if hasattr(c, 'melting_point') and c.melting_point])
    compounds_with_bp = len([c for c in compounds if hasattr(c, 'boiling_point') and c.boiling_point])
...
def export_properties_for_analysis():
    ...
    print(json.dumps(export_data, indent=2)[:500] + "..." if len(str(export_data)) > 500 else json.dumps(export_data, indent=2))
