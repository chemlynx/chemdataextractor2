    ...
    try:
        ...
        for record in doc.records:
            if hasattr(record, "serialize"):
                ...
                records.append({"type": type(record).__name__, "data": str(record)})
...
def batch_process_parallel(
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_path = {
            executor.submit(process_single_document, path): path for path in file_paths
        }
...
def analyze_batch_results(results: list[DocumentResult]) -> dict[str, Any]:
    ...
    if successful_results:
        analysis["summary"]["avg_records_per_doc"] = analysis["summary"]["total_records"] / len(
            successful_results
        )
        analysis["summary"]["avg_processing_time"] = sum(
    ...
    # Display summary
    print(
        f"Documents processed: {analysis['summary']['successful']}/{analysis['summary']['total_documents']}"
    )
...
def performance_comparison():
    ...
    # Compare performance
    speedup = (
        seq_stats.processing_time / par_stats.processing_time
        if par_stats.processing_time > 0
        else 0
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("batch_results"), help="Output directory"
    )
    ...
    try:
        ...
        if args.create_samples or not args.input_dir:
            ...
            file_paths = [
                p
                for p in file_paths
                if p.is_file() and p.suffix in [".txt", ".pdf", ".html", ".xml"]
            ]
