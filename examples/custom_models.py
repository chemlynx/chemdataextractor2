...
from chemdataextractor.model.base import StringType, FloatType, ListType, ModelType
from chemdataextractor.model.units import QuantityModel, DimensionlessModel
from chemdataextractor.model.units.dimension import Dimension
from chemdataextractor.model.units.unit import Unit
from chemdataextractor.parse import R, I, W, Optional as Opt, merge, join
...
class CatalyticActivityModel(QuantityModel):
    ...
    specifier = StringType(
        parse_expression=(
            R('catalytic', group=0) + 
            R('activity|performance|efficiency', group=0)
        )().add_action(join),
        required=True
    )
...
class PhotoluminescenceModel(BaseModel):
    ...
    solvent = StringType(
        parse_expression=(
            I('in') +
            (I('water') | I('methanol') | I('ethanol') | I('acetonitrile') |
             I('dichloromethane') | I('chloroform') | I('toluene') | I('DMSO'))
        )().add_action(join),
        contextual=True
    )
...
class RedoxPotential(BaseModel):
    ...
    assignment = StringType(
        parse_expression=(
            (I('oxidation') | I('reduction') | I('ox') | I('red') |
             R(r'M\+?/?M\d*\+?') | R(r'L/?L\+'))
        )().add_action(join)
    )
    reversible = StringType(
        parse_expression=(
            (I('reversible') | I('irreversible') | I('quasi-reversible') |
             I('rev') | I('irrev'))
        )().add_action(join)
    )
...
class ElectrochemicalModel(BaseModel):
    ...
    technique = StringType(
        parse_expression=(
            (I('CV') | I('cyclic') + I('voltammetry') |
             I('DPV') | I('differential') + I('pulse') + I('voltammetry') |
    ...
    solvent = StringType(
        parse_expression=(
            I('in') +
            (I('acetonitrile') | I('DMF') | I('DMSO') | I('dichloromethane') |
             I('water') | I('methanol') | R(r'CH\d?Cl\d?'))
        )().add_action(join)
    )
    electrolyte = StringType(
        parse_expression=(
            R(r'\d+\.?\d*') + I('M') +
            (I('TBAPF6') | I('TBABF4') | I('LiClO4') | I('KCl') |
    ...
    crystal_system = StringType(
        parse_expression=(
            ...
            (I('cubic') | I('tetragonal') | I('orthorhombic') |
             I('hexagonal') | I('trigonal') | I('monoclinic') | I('triclinic'))
        )().add_action(join)
    )
...
def test_photoluminescence_extraction():
    ...
    for i, record in enumerate(pl_records, 1):
        ...
        for field_name in ['excitation_wavelength', 'emission_wavelength', 
                          'quantum_yield', 'lifetime', 'solvent']:
...
def test_electrochemical_extraction():
    ...
    for i, record in enumerate(ec_records, 1):
        ...
        if record.redox_potentials:
            print(f"  Redox Potentials:")
            for j, redox in enumerate(record.redox_potentials, 1):
                print(f"    {j}. Potential: {redox.potential}, "
...
def test_crystal_structure_extraction():
    ...
    for i, record in enumerate(cs_records, 1):
        print(f"\nCrystal Structure {i}:")
        fields = ['space_group', 'crystal_system', 'unit_cell_a', 
                 'unit_cell_b', 'unit_cell_c', 'density']
    ...
    def validate_photoluminescence(record: PhotoluminescenceModel) -> Dict[str, Any]:
        ...
        if record.excitation_wavelength:
            if not (200 <= record.excitation_wavelength <= 800):
                validation['warnings'].append(f"Unusual excitation wavelength: {record.excitation_wavelength} nm")
        
        if record.emission_wavelength:
            if not (250 <= record.emission_wavelength <= 900):
                validation['warnings'].append(f"Unusual emission wavelength: {record.emission_wavelength} nm")
