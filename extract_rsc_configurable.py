...
def show_available_models():
    ...
    print(f"⚡ Fast models (no BERT): {', '.join([k for k in AVAILABLE_MODELS.keys() if k not in BERT_DEPENDENT_MODELS])}")
...
def extract_configurable_data(file_path, selected_models, model_keys):
    ...
    """
    print(f"🔬 Starting configurable ChemDataExtractor2 analysis")
...
def main():
    ...
    print(f"  'fast'     - Use models without BERT (faster): non-compound models")
    print(f"  'all'      - Use all available models")
    print(f"  'model1,model2' - Specify models by name (comma-separated)")
    print(f"  ''         - Use default models")
