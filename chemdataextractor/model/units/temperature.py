...
units_dict = {
    R(r"°?(((K|k)elvin(s)?)|K)\.?", group=0): Kelvin,
    R(r"(°C|((C|c)elsius))\.?", group=0): Celsius,
    R(r"(\[deg\.] C\.)|([D|d]egrees? C(entigrade(s)?)?\.?)", group=0): Celsius,
    R(r"°?((F|f)ahrenheit|F)\.?", group=0): Fahrenheit,
    R("°|C", group=0): None,
}
