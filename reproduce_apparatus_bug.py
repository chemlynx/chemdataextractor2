...
from chemdataextractor.model import Compound, MeltingPoint, Apparatus
...
def test_direct_merge_scenarios():
    ...
    print(f"Scenario 1: MeltingPoint + Compound(names=['water'])")
    print(f"  Before: apparatus={getattr(mp, 'apparatus', None)}, compound={getattr(mp, 'compound', None)}")

    result = mp.merge_contextual(water_compound)
    print(f"  After: result={result}, apparatus={getattr(mp, 'apparatus', None)}, compound={getattr(mp, 'compound', None)}")
...
def test_step_by_step_merge():
    ...
    print(f"2. Compatibility checks:")
    ...
    print(f"3. Apparatus field analysis:")
    if apparatus_field:
        ...
        if hasattr(apparatus_field, 'model_class'):
            ...
            print(f"   isinstance(compound, apparatus_field.model_class): {isinstance(compound, apparatus_field.model_class)}")

    # Look at compound field
    compound_field = mp.fields.get('compound')
    print(f"4. Compound field analysis:")
    if compound_field:
        ...
        if hasattr(compound_field, 'model_class'):
            ...
            print(f"   isinstance(compound, compound_field.model_class): {isinstance(compound, compound_field.model_class)}")
