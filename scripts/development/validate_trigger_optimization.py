...
from chemdataextractor.parse.optimized_base import OptimizedSentenceParser
    ...
    for doc in documents:
        for sentence in doc.sentences:
            ...
            if parser.trigger_phrase is not None:
                trigger_results = [result for result in parser.trigger_phrase.scan(sentence.tokens)]
...
def main():
    ...
    trigger_speedup = (
        original_results["average_trigger_time_ms"] / optimized_results["average_trigger_time_ms"]
        if optimized_results["average_trigger_time_ms"] > 0
        else 1.0
    )

    overall_speedup = (
        original_results["sentences_per_second"] / optimized_results["sentences_per_second"]
        if original_results["sentences_per_second"] > 0
        else 1.0
    )
    ...
    print("\n✅ Validation complete!")
    print("   Results saved to: trigger_optimization_validation.json")
