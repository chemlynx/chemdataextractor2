...
from chemdataextractor.model.base import InferredProperty
...
class TestModel(unittest.TestCase):
    ...
    def test_serialize(self):
        ...
        self.assertEqual(
            Compound(names=["Coumarin 343"]).serialize(),
            {"Compound": {"names": ["Coumarin 343"]}},
        )
    ...
    def test_model_update_definitions(self):
        ...
        results = [i for i in NeelTemperature.parsers[0].parse_sentence(test_sentence)][
            0
        ].serialize()
    ...
    def test_is_empty(self):
        ...
        non_empty_records = [
            OuterModel(complex_list=[MeltingPoint()]),
    ...
    def test_inferred_property(self):
        ...
        class OuterModel(BaseModel):
            inferred_from_field = InferredProperty(
                StringType(), origin_field="origin_field", inferrer=reverse_string
            )
            inferred_from_nested = InferredProperty(
                StringType(),
                origin_field="origin_model.string_field",
    ...
    def test_deserialize(self):
        ...
        outer_model = OuterModel(origin_model=inner_model, origin_field="abcd")
        self.assertEqual(
            outer_model.serialize(),
            OuterModel.deserialize(outer_model.serialize()).serialize(),
        )
        expected = {
            "OuterModel": {
                "origin_field": "abcd",
