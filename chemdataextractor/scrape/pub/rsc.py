...
strip_rsc_html = Cleaner(
    strip_xpath=".//*",
    kill_xpath='.//span[@class="sup_ref"]|.//a[text()="†"]|.//i/small/sup/a|.//*[@style="display:none;"]',
)
...
def parse_rsc_html(htmlstring):
    ...
    root = fromstring(
        htmlstring,
        parser=HTMLParser(recover=True, encoding=converted.original_encoding),
    )
    ...
    for child in root.get_element_by_id("wrapper"):
        if newp is not None:
            if (
                ...
                or child.get("id", "").startswith("sect")
                or child.getnext() is None
            ):
...
def space_references(document):
    ...
    for ref in document.xpath('.//a/sup/span[@class="sup_ref"]'):
        ...
        if a is not None:
            atail = a.tail or ""
            if (
                not atail.startswith(")")
                and not atail.startswith(",")
                and not atail.startswith(" ")
            ):
                a.tail = " " + atail
    return document
...
class RscSearchDocument(Entity):
    ...
    process_html_url = Chain(
        normalize,
        LStrip(":///en/content/articlepdf/"),
        LAdd("https://pubs.rsc.org/en/content/articlehtml/"),
    )
...
class RscLandingDocument(DocumentEntity):
    ...
    supplements = EntityField(
        RscLandingSupplement,
        ".ESIright_highlight_txt_red, .ESIright_highlight_txt_red_hidden",
        all=True,
    )
    # TODO: The meta abstract and title inherited from DocumentEntity are missing characters

    process_abstract = Chain(
        normalize,
        Discard(
...
class RscChemicalMention(Entity):
    ...
    process_chemspider_id = Chain(
        LStrip("http://www.chemspider.com/Chemical-Structure."),
        RStrip(".html"),
        Discard(""),
    )
    process_inchi = Chain(
        LStrip("http://www.chemspider.com/Search.aspx?q="),
