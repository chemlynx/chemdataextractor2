    ...
    for i, mp in enumerate(melting_points, 1):
        ...
        if mp.value:
            ...
            if temp_value < -273:  # Below absolute zero
                print("    ⚠️  WARNING: Temperature below absolute zero")
            elif temp_value > 1000:  # Unrealistically high for organic compounds
                print("    ⚠️  WARNING: Unusually high temperature")
            else:
                print("    ✅ Temperature appears reasonable")
        else:
            print("    ❌ No valid temperature extracted")
