...
def analyze_rsc_html(file_path):
    ...
    compound_labels = re.findall(r'\b(?:compound|product|intermediate)\s*(\d+[a-z]?|\([0-9]+[a-z]?\))', text_content, re.IGNORECASE)
    results['compounds_found'] = list(set(compound_labels))
    print(f"Found {len(results['compounds_found'])} compound labels: {results['compounds_found'][:10]}")

    # Look for chemical names (simplified patterns)
    quinazoline_compounds = re.findall(r'\b\d+[a-z]?[,-]\s*[A-Za-z][^.]*?quinazolin[^.]*', text_content)
    ...
    print(f"Found {len(results['measurements_found'])} melting point values: {results['measurements_found']}")
    ...
    experimental_sections = re.findall(r'(experimental[^.]*|synthesis[^.]*|procedure[^.]*)', text_content, re.IGNORECASE)
...
def main():
    ...
    if save_analysis_results(results, output_file):
        ...
        print(f"  • Rich experimental data detected in {results['file_info']['text_length']:,} characters")

        print(f"\n💡 This RSC publication contains substantial chemical data suitable for ChemDataExtractor2 processing.")
        print(f"📈 The full extraction (when completed) should yield significant experimental data.")
