...
class WordTokenizer(BaseTokenizer):
    ...
    def _subspan(self, s, span, nextspan, additional_regex):
        ...
        if (
            span[1] - span[0] < 2
            or text in self.SPLIT
            or text in self.SPLIT_END_WORD
            or text in self.SPLIT_START_WORD
            or lowertext in self.NO_SPLIT
        ):
            return [span]
...
class ChemWordTokenizer(WordTokenizer):
    ...
    def _subspan(self, s, span, nextspan, additional_regex):
        ...
        if (
            text.startswith("(")
            and text.endswith(")")
            and self._closing_bracket_index(text) == len(text) - 1
        ):
        ...
        for i, char in enumerate(text):
            ...
            if char in {":", ";"}:
                ...
                if not (
                    before
                    and after
                    and before[-1] in self.NO_SPLIT_SLASH
                    and after[0] in self.NO_SPLIT_SLASH
                ):
                    return self._split_span(span, i, 1)
            elif char == ">":
                ...
                if lowerafter[:7] == "of-the-":
                    return [
                        (span[0], span[0] + i),
                        (span[0] + i, span[0] + i + 1),
                        (span[0] + i + 1, span[0] + i + 3),
                        (span[0] + i + 3, span[0] + i + 4),
                        (span[0] + i + 4, span[0] + i + 7),
                        (span[0] + i + 7, span[0] + i + 8),
                        (span[0] + i + 8, span[1]),
                    ]
                if lowerafter[:5] in {"on-a-", "of-a-"}:
                    return [
                        ...
                        (span[0] + i + 1, span[0] + i + 3),
                        (span[0] + i + 3, span[0] + i + 4),
                        (span[0] + i + 4, span[0] + i + 5),
                        (span[0] + i + 5, span[0] + i + 6),
                        (span[0] + i + 6, span[1]),
                    ]
...
class FineWordTokenizer(WordTokenizer):
    ...
    def _subspan(self, s, span, nextspan, additional_regex):
        ...
        for i, char in enumerate(text):
            if i < len(text) - 1:
                nextchar = text[i + 1]
                if (char in GREEK and nextchar not in GREEK) or (
                    char not in GREEK and nextchar in GREEK
                ):
...
class BertWordTokenizer(ChemWordTokenizer):
    ...
    def span_tokenize(self, s, additional_regex=None):
        ...
        while i < len(zipped):
            ...
            if (
                s[offset[0] : offset[1]] in self.do_not_split_if_in_num
        ...
        if additional_regex is not None:
            i = 0
            while i < len(spans):
                subspans = self.handle_additional_regex(
                    s,
                    spans[i],
                    spans[i + 1] if i + 1 < len(spans) else None,
                    additional_regex,
