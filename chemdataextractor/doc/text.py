...
from ..nlp.lexicon import Lexicon  # noqa: F401
from ..nlp.new_cem import CemTagger
from ..nlp.pos import ChemCrfPosTagger
from ..nlp.pos import CrfPosTagger  # noqa: F401
...
from ..nlp.tokenize import SentenceTokenizer  # noqa: F401
from ..nlp.tokenize import WordTokenizer  # noqa: F401
...
class BaseText(BaseElement):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.abbreviation_detector = (
            ...
            if abbreviation_detector is not None
            else self.abbreviation_detector
        )
        if taggers is not None and len(taggers) != 0:
            self.taggers = taggers

    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(id={self.id}, references={self.references}, text={self._text}"
    ...
    @deprecated(
        deprecated_in="2.1",
        details="Deprecated due to the introduction of taggers API. Consult the migration guide for more information.",
    )
...
class Text(collections.abc.Sequence, BaseText):
    ...
    @deprecated(
        deprecated_in="2.1",
        details="Deprecated due to the introduction of RichTokens, and is now just an alias for .tokens.",
    )
    ...
    def records(self):
        ...
        records = ModelList(
            *[r for sentence_records in records_by_sentence for r in sentence_records]
        )
...
class Citation(Text):
    taggers = [
        ChemCrfPosTagger(),
        NoneTagger(tag_type=NER_TAG_TYPE),
        NoneTagger(tag_type="dependency"),
        IndexTagger(),
    ]
    abbreviation_detector = None
...
class Sentence(BaseText):
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}({self._text}, {self.start}, {self.end})"
    ...
    def ner_tagged_tokens(self):
        ...
        from the sentence.
        """
        return list(zip(self.raw_tokens, self.ner_tags, strict=False))
    ...
    def ner_tags(self):
        ...
        abbrev_defs = (
            self.document.abbreviation_definitions
            if self.document
            else self.abbreviation_definitions
        )
        ...
        for i in range(0, len(ner_tags)):
            for abbr, long, ner_tag in abbrev_defs:
                if abbr == self.raw_tokens[i : i + len(abbr)]:
                    ...
                    ner_tags[i] = f"B-{ner_tag}" if ner_tag is not None else None
                    ner_tags[i + 1 : i + len(abbr)] = [
                        f"I-{ner_tag}" if ner_tag is not None else None
                    ] * (len(abbr) - 1)
                    ...
                    if old_ner_tags != ner_tags[i : i + len(abbr)]:
                        log.debug(
                            f"Correcting abbreviation tag: {' '.join(abbr)} ({' '.join(long)}): {old_ner_tags} -> {ner_tags[i : i + len(abbr)]}"
                        )
    ...
    def cems(self):
        ...
        for result in chemical_name.scan(self.tokens):
            ...
            for bpair in [("(", ")"), ("[", "]")]:
                if (
                    len(currenttext) > 2
                    and currenttext[0] == bpair[0]
                    and currenttext[-1] == bpair[1]
                ):
                    level = 1
            ...
            if len(comps) > 1:
                for split in SPLITS:
                    if all(re.search(split, currenttext[comp[0] : comp[1]]) for comp in comps):
                        ...
                        for comp in comps:
                            span = Span(
                                text=currenttext[comp[0] : comp[1]],
                                start=start + comp[0],
                                end=start + comp[1],
                            )
                ...
                for special in SPECIALS:
                    ...
                    if m:
                        ...
                        for i in range(1, len(m.groups()) + 1):
                            span = Span(
                                text=m.group(i),
                                start=split_span.start + m.start(i),
                                end=split_span.start + m.end(i),
                            )
    ...
    def chemical_definitions(self):
        ...
        for result in cem_phrase.scan(tokens):
            ...
            if name and label:
                cem_def = {"name": name, "label": label, "start": start, "end": end}
    ...
    def _assign_tags(self, tag_type):
        ...
        for tagger in reversed(self.taggers):
            if tagger.can_tag(tag_type):
                tags = None
                if (
                    hasattr(tagger, "batch_tag_for_type")
                    and tagger.can_batch_tag(tag_type)
    ...
    def from_tdecell(cls, tde_cell, document=None, **kwargs):
        ...
        text = " ".join(
            [
                tde_cell[0],
                separator,
                cell_separator.join(tde_cell[1]),
                separator,
                cell_separator.join(tde_cell[2]),
            ]
        )
...
class Span:
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}({self.text}, {self.start}, {self.end})"
        ...
        if name in self._tags:
            ...
            if name not in self._tags:
                raise AttributeError(
                    name
                    + " is not a supported tag type for the sentence: "
                    + str(self.sentence)
                    + str(self.sentence.taggers)
                    + str(type(self.sentence))
                    + str(self._tags)
                    + str(self)
                )
            return self._tags[name]
