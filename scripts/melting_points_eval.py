...
from chemdataextractor.parse import ChemicalLabelParser
from chemdataextractor.parse import CompoundParser
from chemdataextractor.parse import MpParser
from chemdataextractor.text import HYPHENS
from chemdataextractor.text import MINUSES
...
def float_value(value):
    ...
    if " to " in value:
        ...
        if re.match(r"^\d\d\d\d$", start) and re.match(r"^\d\d\d$", end):
            print("MISSING DECIMAL")
            print(start)
            start = start[:3] + "." + start[3]
            print(start)
        if re.match(r"^\d\d\d\d$", end) and re.match(r"^\d\d\d$", start):
            print("MISSING DECIMAL")
            print(end)
            end = end[:3] + "." + end[3]
            print(end)
