...
class LxmlReader(BaseReader, metaclass=ABCMeta):
    ...
    def _parse_element_r(
        ...
        for child in el:
            ...
            if (
                child.tag not in {etree.Comment, etree.ProcessingInstruction}
                and child.tag.lower() == "br"
            ):
                elements.append(element_cls(""))

            child_elements = self._parse_element_r(
                child, specials=specials, refs=refs, id=id, element_cls=element_cls
            )
            if (
            ...
            if child.tail is not None:
                if (
                    self._is_inline(child)
                    and len(elements) > 0
                    and isinstance(elements[-1], element_cls)
                ):
    ...
    def _parse_table(
        ...
        hrows = self._parse_table_rows(
            self._css(self.table_head_row_css, el), refs=refs, specials=specials
        )
        rows = rows = self._parse_table_rows(
            self._css(self.table_body_row_css, el), refs=refs, specials=specials
        )
        data = []
        for hr in hrows:
    ...
    def parse(self, fstring: str | bytes) -> Document:
        ...
        for citation in citations:
            specials[citation] = self._parse_text(
                citation, element_cls=Citation, refs=refs, specials=specials
            )
        for md in metadata:
