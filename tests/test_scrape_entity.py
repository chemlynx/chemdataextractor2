...
from chemdataextractor.scrape.fields import EntityField
from chemdataextractor.scrape.fields import StringField
from chemdataextractor.scrape.selector import Selector
from chemdataextractor.text.processors import Chain
from chemdataextractor.text.processors import RAdd
...
class TestMultiScrape(unittest.TestCase):
    ...
    def setUp(self):
        self.blogs = Blog.scrape(
            Selector.from_text(ARTICLE_HTML), root="/html/body/div", xpath=True
        )
...
class TestComplexScrape(unittest.TestCase):
    ...
    def setUp(self):
        self.scrapes = Article.scrape(
            Selector.from_text(COMPLEX_HTML), root="/html/body/div", xpath=True
        )
    ...
    def test_serialize(self):
        """Test serialization of results to a list of dictionaries."""
        self.assertEqual(
            self.scrapes.serialize(),
            [
                {
                    "content": ["First para", "Second para"],
                    "title": "First article",
                    "authors": [{"lastname": "Swain", "firstname": "Matt"}],
                },
                {
                ...
                {
                    "content": ["Thing one", "Thing two"],
                    "title": "Third article",
                    "authors": [{"lastname": "Bond", "firstname": "James"}],
                },
            ],
        )
