...
from chemdataextractor.nlp.tokenize import FineWordTokenizer
...
class TestChemTokenizer(unittest.TestCase):
    ...
    def test_tokenise_model(self):
        ...
        self.assertEqual(
            ["The", "pressure", "was", "measured", "to", "be", "12", "MPa"],
            [token.text for token in tokens],
        )
    ...
    def test_bracket4(self):
        ...
        self.assertEqual(
            [
                ...
                "of",
                "3,21-diacetoxy-5",
                "beta,19-cyclopregna-2,9(11)-diene-4,20-dione",
                "(",
                "10",
                ")",
            ],
    ...
    def test_comma1(self):
        ...
        self.assertEqual(
            [
                ...
                "to",
                "12-acetoxy-11",
                "beta,19-epoxypregn-5-ene-4,20-dione",
                "(",
                "8",
                ")",
                ".",
    ...
    def test_curly_bracket(self):
        ...
        self.assertEqual(
            [
                ...
                "complexes",
                ",",
                "{[Mn(salpn)]2[Fe(CN)5NO]}n",
                "(",
                "1",
                ")",
                ",",
                "{[Mn(salpn)(CH3OH)]4[Mn(CN)5NO]}[C(CN)3]·3H2O",
                "(",
                "2",
                ")",
                ",",
                "{[Mn(dapsc)][Fe(CN)5NO]·0.5CH3OH·0.25H2O}n",
                "(",
                "3",
                ")",
                "and",
                "{[Mn(salpn)(CH3OH)]4[Fe(CN)5NO]}(ClO4)2·4H2O",
                "(",
                "4",
                ")",
                ",",
                ...
                "salpn2−",
                "=",
                "N,N′-1,3-propylene-bis(salicylideneiminato)",
                "dianion",
                "and",
                "dapsc",
                "=",
                "2,6-diacetylpyridine-bis(semicarbazone)",
            ],
            self.t.tokenize(
    ...
    def test_bracket_subsequent(self):
        ...
        self.assertEqual(
            [
                "The",
                "α-ammonium-acylchloride",
                "salts",
                "[NH2(CH2)3CHC(O)Cl][WOCl5]",
                ",",
                "1a",
                ",",
                "and",
                "[MeNH2CH2C(O)Cl][WOCl5]",
                ",",
                "1b",
                ".",
            ],
    ...
    def test_symbols(self):
        ...
        self.assertEqual(
            ...
            self.t.tokenize("(H2A <--> HA- + H+ and HA- <--> A= + H+)"),
        )
    ...
    def test_sentence_end(self):
        self.assertEqual(
            ["upon", "addition", "of", "Ni(II)", ";"],
            self.t.tokenize("upon addition of Ni(II);"),
        )
        self.assertEqual(
            ["upon", "addition", "of", "Ni(II)", "."],
            self.t.tokenize("upon addition of Ni(II)."),
        ...
        self.assertEqual(
            ["space", "group", "P", "(", "No.", "2", ")", "."],
            self.t.tokenize("space group P (No. 2)."),
        )
    ...
    def test_abbreviations(self):
        ...
        self.assertEqual(
            ...
            self.t.tokenize("Elements, e.g. calcium"),
        )
    ...
    def test_more_brackets(self):
        ...
        self.assertEqual(
            ["buffer", "(", "pH", "7.4", ")", "."], self.t.tokenize("buffer (pH 7.4).")
        )
    ...
    def test_saccharide(self):
        ...
        self.assertEqual(
            ...
            self.t.tokenize("α-D-Glucopyranosyl-(1→4)-β-D-glucopyranose"),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            self.t.tokenize("((1→2)-α-D-galacto)-(1→4)-β-D-Glucan"),
        )
    ...
    def test_hyphen_twice(self):
        ...
        self.assertEqual(
            ["State", "-", "of", "-", "the", "-", "art"],
            self.t.tokenize("State-of-the-art"),
        )
    ...
    def test_bracket_hyphen(self):
        ...
        self.assertEqual(
            ...
            self.t.tokenize("(nano LC/nano-ESI-IT-MS)"),
        )
    ...
    def test_hyphen_nosplit(self):
        ...
        self.assertEqual(
            ["3,3′-dianisyl-substituted"], self.t.tokenize("3,3′-dianisyl-substituted")
        )
    ...
    def test_hyphen_split(self):
        ...
        self.assertEqual(
            ...
            self.t.tokenize("methicillin-resistant,"),
        )
    ...
    def test_ms(self):
        self.assertEqual(
            ...
            self.t.tokenize(
                "[M+H]+ 1523.86, [M+2H]2+ 762.43, [M+3H]3+ 508.62. Observed: [M+H]+ 1523.20, [M+2H]2+ 762.45, [M+3H]3+ 508.70."
            ),
        )
        # This isn't ideal but can't see any alternative apart from super fine tokenization
        self.assertEqual(
            ["527.3596", "[", "M", "+", "H]+", ",", "C30H47N4O4+"],
            self.t.tokenize("527.3596 [M + H]+, C30H47N4O4+"),
        )

    def test_quantities(self):
        self.assertEqual(
            ["contamination", "of", "2", "%", "Cl2"],
            self.t.tokenize("contamination of 2% Cl2"),
        )
        self.assertEqual(
            ["Placed", "at", "a", "distance", "of", "7.2", "cm", "."],
            self.t.tokenize("Placed at a distance of 7.2cm."),
        )
        self.assertEqual(
            ["Addition", "of", "~", "1.8", "mg", "of", "CaCO3", "."],
            self.t.tokenize("Addition of ~1.8mg of CaCO3."),
        )
        self.assertEqual(
            ["Recorded", "in", "HCl", "(", "pH", "2", ")", "."],
            self.t.tokenize("Recorded in HCl (pH2)."),
        )
        self.assertEqual(
            ["Experienced", "a", "pressure", "of", "160", "kPa", "."],
            self.t.tokenize("Experienced a pressure of 160kPa."),
        )
        self.assertEqual(
            ["Brought", "to", "pH", "10.5", ",", "gradually", "."],
            self.t.tokenize("Brought to pH10.5, gradually."),
        )
        self.assertEqual(
            ["A", "volume", "of", "24", "cm3", "was", "drained", "."],
            self.t.tokenize("A volume of 24cm3 was drained."),
        )
        self.assertEqual(
            ["2", "M", "H2SO4", "was", "heated", "."],
            self.t.tokenize("2M H2SO4 was heated."),
        )
        self.assertEqual(
            ["The", "spectrum", "was", "recorded", "at", "10", "°", "C"],
            self.t.tokenize("The spectrum was recorded at 10° C"),
        )
        self.assertEqual(
            ["The", "spectrum", "was", "recorded", "at", "10", "°C"],
            self.t.tokenize("The spectrum was recorded at 10°C"),
        ...
        self.assertEqual(
            ["Added", "3.5", "g", "and", "stirred", "for", "5.5", "s", "."],
            self.t.tokenize("Added 3.5g and stirred for 5.5s."),
        )
        ...
        self.assertEqual(
            ...
            self.t.tokenize("In the 1980s there was"),
        )
        self.assertEqual(
            ["Produced", "compounds", "3g", ",", "3l", ",", "3m", "and", "3n"],
            self.t.tokenize("Produced compounds 3g, 3l, 3m and 3n"),
        )
        self.assertEqual(
            ["9.66", "(", "d", ",", "1H", ",", "3J", "=", "5.4", "Hz", ",", "H15", ")"],
            self.t.tokenize("9.66 (d, 1H, 3J = 5.4Hz, H15)"),
        )
        self.assertEqual(
            [
                "greater",
            ...
            self.t.tokenize("greater than 3 × 10-11mol kg-1 or 2 × 10-5mol kg-1"),
        )
    ...
    def test_bracket_chems(self):
        self.assertEqual(
            ["molecules", "of", "the", "[NiL2]", "complex"],
            self.t.tokenize("molecules of the [NiL2] complex"),
        )
...
class TestFineWordTokenizer(unittest.TestCase):
    ...
    def test_bracket_chems(self):
        ...
        self.assertEqual(
            ["[", "Et3NBz", "]", "[", "FeIIICl4", "]"],
            self.t.tokenize("[Et3NBz][FeIIICl4]"),
        )
        self.assertEqual(
            ["[", "2PA", "-", "Mmim", "]", "[", "Tf2N", "]"],
        ...
        self.assertEqual(
            ["(", "Ph3PO", ")", "(", "Ph3POH", ")", "(", "HSO4", ")"],
            self.t.tokenize("(Ph3PO)(Ph3POH)(HSO4)"),
        )
        self.assertEqual(["(", "iron", "(", "III", ")"], self.t.tokenize("(iron(III)"))
    ...
    def test_chem_formula(self):
        ...
        self.assertEqual(
            ["[", "Al", "(", "H2L", ")", "n", "]", "3", "-"],
            self.t.tokenize("[Al(H2L)n]3-"),
        )
        self.assertEqual(
            ["[", "Fe", "(", "CN", ")", "5", "(", "NO", ")", "]", "2", "-"],
            self.t.tokenize("[Fe(CN)5(NO)]2-"),
        )
        self.assertEqual(
            ["[", "Fe", "(", "CN", ")", "5", "(", "NO", ")", "]", "2", "−"],
            self.t.tokenize("[Fe(CN)5(NO)]2−"),
